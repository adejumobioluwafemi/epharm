...
Async service methods are called via asyncio.run() — no pytest-asyncio needed.
"""
...
class TestSendWelcomeEmail:
    ...
    def test_send_welcome_email_returns_success_when_emails_disabled(
        ...
        result = asyncio.run(EmailService.send_welcome_email(welcome_data))
        assert result.success is True

    def test_send_welcome_email_does_not_raise(
        self, welcome_data: WelcomeEmailData, mock_email_settings
    ):
        """No exception must be raised regardless of email configuration."""
        try:
            asyncio.run(EmailService.send_welcome_email(welcome_data))
        except Exception as exc:
            pytest.fail(f"send_welcome_email raised unexpectedly: {exc}")

    def test_send_welcome_email_result_has_required_fields(
        self, welcome_data: WelcomeEmailData, mock_email_settings
    ):
...
class TestSendPasswordResetEmail:
    ...
    def test_send_password_reset_email_returns_success_when_emails_disabled(
        self, password_reset_data: PasswordResetEmailData, mock_email_settings
    ):
        result = asyncio.run(EmailService.send_password_reset_email(password_reset_data))
        assert result.success is True

    def test_send_password_reset_email_does_not_raise(
        self, password_reset_data: PasswordResetEmailData, mock_email_settings
    ):
        try:
            asyncio.run(EmailService.send_password_reset_email(password_reset_data))
        except Exception as exc:
            pytest.fail(f"send_password_reset_email raised unexpectedly: {exc}")

    def test_send_password_reset_email_result_has_required_fields(
        self, password_reset_data: PasswordResetEmailData, mock_email_settings
    ):
        result = asyncio.run(EmailService.send_password_reset_email(password_reset_data))
        assert hasattr(result, "success")
...
class TestSendPasswordChangedEmail:
    ...
    def test_send_password_changed_email_returns_success_when_emails_disabled(
        self, password_changed_data: PasswordChangedEmailData, mock_email_settings
    ):
        result = asyncio.run(EmailService.send_password_changed_email(password_changed_data))
        assert result.success is True

    def test_send_password_changed_email_does_not_raise(
        self, password_changed_data: PasswordChangedEmailData, mock_email_settings
    ):
        try:
            asyncio.run(EmailService.send_password_changed_email(password_changed_data))
        except Exception as exc:
...
class TestSendAccountLockedEmail:
    ...
    def test_send_account_locked_email_returns_success_when_emails_disabled(
        self, account_locked_data: AccountLockedEmailData, mock_email_settings
    ):
        result = asyncio.run(EmailService.send_account_locked_email(account_locked_data))
        assert result.success is True

    def test_send_account_locked_email_does_not_raise(
        self, account_locked_data: AccountLockedEmailData, mock_email_settings
    ):
        try:
            asyncio.run(EmailService.send_account_locked_email(account_locked_data))
        except Exception as exc:
